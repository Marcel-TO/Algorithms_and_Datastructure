//-----------------------------------------------------------------------
// <copyright file="DivideCommand.cs" company="FHWN">
//     Copyright (c) Marcel Turobin-Ort. All rights reserved.
// </copyright>
// <author>Marcel Turobin-Ort</author>
// <summary>Defines the divide command of the befunge program interpreter.</summary>
//-----------------------------------------------------------------------
namespace BefungeInterpreter.Commands
{
    using System;
    using BefungeInterpreter.Interfaces;
    using BefungeInterpreter.Logic;

    /// <summary>
    /// Defines the divide command of the <see cref="BefungeProgram"/> interpreter.
    /// </summary>
    public class DivideCommand : BaseCommand
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="DivideCommand"/> class.
        /// </summary>
        /// <param name="program">The current <see cref="BefungeProgram"/> that gets interpreted.</param>
        public DivideCommand(BefungeProgram program) : base("/", "divide", program)
        {
        }

        /// <summary>
        /// Represents the visitor pattern for command execution.
        /// </summary>
        /// <param name="visitor">The used visitor interface.</param>
        public override void Accept(ICommandVisitor visitor)
        {
            visitor.Visit(this);
        }

        /// <summary>
        /// Represents the execution method of the current command.
        /// </summary>
        /// <param name="program">The <see cref="BefungeProgram"/> that gets interpreted.</param>
        /// <param name="logger">The current logger of the application.</param>
        public void Execute(BefungeProgram program, ILogger logger)
        {
            // Checks if the parameter is null.
            if (program == null)
            {
                throw new ArgumentNullException($"The {nameof(program)} must not be null.");
            }
            else if (logger == null)
            {
                throw new ArgumentNullException($"The {nameof(logger)} must not be null.");
            }

            //// Checks if there are no values to be popped.
            //if (program.Stack.Count == 0)
            //{
            //    program.StackPush(0);
            //    return;
            //}

            // Pops the values.
            int value2 = program.StackPop();
            int value1 = program.StackPop();

            // Checks if values are zero to avoid dividing by 0.
            if (value2 == 0)
            {
                logger.Clear();
                logger.Log("It appears the interpreter would divide by 0. What should be the result?");
                logger.Continue();
                program.StackPush(logger.GetUserIntInput());
                logger.ShowProgramContent(program.Content, program.Position, program.ValueList, program.Output);
                return;
            }

            // Divides the values and pushes the result on the stack.
            int res = value1 / value2;
            program.StackPush(res);
        }
    }
}
